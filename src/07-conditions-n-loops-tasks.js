...
function doRectanglesOverlap(rect1, rect2) {
  const result = (rect1.top > (rect2.top + rect2.height))
...
function getDigitalRoot(num) {
  const sum = +num.toString().split('').reduce((reducer, item) => {
    let y = reducer;
    y += +item;
    return y;
  }, 0);
...
function getMatrixProduct(m1, m2) {
  const result = [];
  const filler = [];
  filler.length = m2[0].length;
  filler.fill(0);
  m1.forEach(() => result.push([...filler]));
...
function evaluateTicTacToePosition(position) {
  ...
  if (p[2][0] && (p[2][0] === p[1][1]) && (p[2][0] === p[0][2])) return p[2][0];
  return undefined;
}
